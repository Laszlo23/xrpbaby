#!/usr/bin/env bash
# Static SPA for eco.buildingcultureid.space — root + try_files → index.html.
# Run after rsync of Vite dist to REMOTE_ROOT.
#
#   export DEPLOY_HOST=root@your.vps.ip
#   export PUBLIC_DOMAIN=eco.buildingcultureid.space
#   export REMOTE_ROOT=/var/www/eco-buildingculture
#   ./scripts/install-nginx-eco-static-on-server.sh
#
set -euo pipefail
HOST="${DEPLOY_HOST:?set DEPLOY_HOST}"
DOMAIN="${PUBLIC_DOMAIN:-eco.buildingcultureid.space}"
...
EXIST="/etc/nginx/sites-available/buildingculture-eco.conf"
...
cat >"$TMP" <<EOF
# b3 install-nginx-eco-static-on-server.sh
server {
    listen 80;
    listen [::]:80;
    server_name ${DOMAIN};
...
OUT=/etc/nginx/sites-available/buildingculture-eco.conf
if [[ -d /etc/nginx/sites-available ]]; then
  sudo cp "$TMP" "$OUT"
  sudo ln -sf "$OUT" /etc/nginx/sites-enabled/buildingculture-eco.conf
elif [[ -d /etc/nginx/conf.d ]]; then
  sudo cp "$TMP" /etc/nginx/conf.d/buildingculture-eco.conf
else
...
sudo nginx -t
sudo systemctl reload nginx 2>/dev/null || sudo service nginx reload
echo "Installed eco static site block."
REMOTE
