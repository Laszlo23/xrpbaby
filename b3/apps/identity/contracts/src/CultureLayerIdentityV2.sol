...
/// @title CultureLayerIdentityV2 — native ETH mint + BCC rail at 11.11% discount
contract CultureLayerIdentityV2 is ERC721, Ownable {
    ...
    uint256 public usdMintPriceE6 = 1_110_000;
    ...
    constructor(
        ...
        address bccOracle_,
        address treasury_
    ) ERC721("Culture Layer Identity", "CLID") Ownable(initialOwner) {
    ...
    function setUsdMintPriceE6(uint256 usdE6) external onlyOwner {
        usdMintPriceE6 = usdE6;
    }
    ...
    function mint(string calldata handle, uint8 tldId) external payable returns (uint256 tokenId) {
        ...
        return _mintIdentity(handle, tldId);
    }

    /// @notice Mint paying BCC at 11.11% discount vs USD list price (via oracle).
    function mintWithBcc(string calldata handle, uint8 tldId) external returns (uint256 tokenId) {
        if (!isValidTld(tldId)) revert InvalidTld();
        _validateHandle(handle);
        uint256 full = bccOracle.bccAmountForUsd(usdMintPriceE6);
        uint256 cost = (full * (10_000 - BCC_DISCOUNT_BPS)) / 10_000;
        bccToken.safeTransferFrom(msg.sender, treasury, cost);
        return _mintIdentity(handle, tldId);
    }

    function quoteMintWithBcc() external view returns (uint256 bccCost) {
        uint256 full = bccOracle.bccAmountForUsd(usdMintPriceE6);
        bccCost = (full * (10_000 - BCC_DISCOUNT_BPS)) / 10_000;
    }

    function _mintIdentity(string calldata handle, uint8 tldId) internal returns (uint256 tokenId) {
