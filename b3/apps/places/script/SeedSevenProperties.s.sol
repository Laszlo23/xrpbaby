...
/// @notice Registers 7 demo parcels and one PropertyShareToken each — for **fresh** registries (no prior registerProperty).
/// @dev If you already ran SeedThreeProperties, use SeedFourMoreProperties.s.sol instead.
///      USD values match `web/src/lib/demo-properties.ts` `illustrativePropertyValueUsd` per property order; initial mint = full cap to treasury.
contract SeedSevenPropertiesScript is Script {
    function run() external {
        ...
        for (uint256 i; i < 7; ++i) {
            ...
            string memory uri = string.concat("https://buildingculture.demo/meta/at-", _toString(i + 1), ".json");
