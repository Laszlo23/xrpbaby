# BCC on-chain launch — tokenomics defaults (Phase 0)
...
|-----------|----------------|
| **Cap** | `1_000_000 ether` (1M BCC) — matches `DeployBCD.s.sol` / `BCD_CAP` env |
...
- **USDC (Base mainnet):** set `paymentToken` to `0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913` and use `paymentPerWholeBcd` in **6-decimal** USDC units per 1e18 BCC wei (see contract NatSpec).
...
- **`maxBcdWei`** — max cumulative BCC (18-decimal wei) purchasable in that round by this wallet.
