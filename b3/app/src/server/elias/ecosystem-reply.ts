...
import { formatTouchpointsForPromptDbPreferred } from "@/server/elias/touchpoints";
...
export async function runEliasEcosystemReply(input: {
  ...
  const { contextBlock } = await retrieveBcCorpus(input.userMessage, { topK: 6 });
  const touch = await formatTouchpointsForPromptDbPreferred();
  ...
  const sys =
    new SystemMessage(`You are **Elias** — Building Culture's ecosystem operator (not a generic chatbot).
