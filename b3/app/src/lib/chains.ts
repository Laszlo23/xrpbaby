...
export function getAllWagmiChains(): readonly WagmiChain[] {
  ...
  const core = [base, bsc, b3Testnet, b3Mainnet] as const;
  const middle = includeWorldChainInWallet() ? ([worldChain, ...core] as const) : core;
  if (net === "base-sepolia") {
    return [baseSepolia, ...middle] as unknown as readonly WagmiChain[];
  }
  return middle as unknown as readonly WagmiChain[];
}
...
/** World Chain (World App native L2, EIP-155 chain id 480). RPC override via `VITE_WORLD_CHAIN_RPC_URL`. */
