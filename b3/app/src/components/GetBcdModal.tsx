...
export function GetBcdModal() {
  ...
  const roundActive =
    !!round && isRoundConfigured && nowSec >= round.start && nowSec <= round.end && !salePaused;
  ...
  const needsApprove =
    !isEthSale && payQuote && allowance !== undefined && (allowance as bigint) < payQuote.totalWei;
        ...
        <div className="space-y-4 py-2">
          {saleEnabled && round ? (
              ...
              {!isRoundConfigured ? (
                ...
                <>
                  <p className="text-xs text-zinc-500 leading-relaxed">
                    ...
                    · {isEthSale ? "Pay in ETH" : `Pay in ${(paySymbol as string) || "ERC-20"}`} ·
                  ...
                  {soldWei !== undefined ? (
                    <p className="font-mono text-[11px] text-zinc-500">
                      Round sold {formatUnits(soldWei as bigint, 18)} /{" "}
                      {formatUnits(round.maxBcdWei, 18)} {BCD_SYMBOL}
                    ...
                    <div className="space-y-2">
                      ...
                      {eligLoading ? (
                        ...
                        <p className="text-xs text-emerald-600/90">
                          Loaded max{" "}
                          {merkleMaxWei !== undefined ? formatUnits(merkleMaxWei, 18) : "—"}{" "}
                          {BCD_SYMBOL} allowance for this round.
                        </p>
                      )}
            ...
            <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4">
              ...
              <p className="mt-2 text-center text-[11px] text-zinc-600">
                Rate: {ratio.toString()} {BCD_SYMBOL} per 1 ETH (display preview when no sale
                address).
              </p>
            </div>
          )}
          {!token ? (
            <p className="text-sm text-zinc-500">
              ...
              <strong className="text-zinc-300">{getDemoBcdBalanceDisplay()}</strong> {BCD_SYMBOL}{" "}
              in the UI. Set <span className="font-mono">VITE_BCD_TOKEN_ADDRESS</span> for live
              reads.
            </p>
          ) : null}
          {genesisReady ? (
            <p className="text-sm leading-relaxed text-zinc-500">
              <strong className="text-zinc-200">Genesis is live:</strong> claim your whitelist
              allocation on the Mission page (merkle proof).
            </p>
          ) : null}
        </div>
          ...
          {!saleEnabled ? (
            <Button
              ...
              title={
                canBuyOnChain
                  ? undefined
                  : "Connect sale contract via VITE_BCD_SALE_ADDRESS when deployed"
              }
            >
