...
export function BccBridgePanel({ compact = false }: BccBridgePanelProps) {
    ...
    return (
      <div className={panelClass}>
        <p className="text-xs text-zinc-400">
          Cross-chain bridge activates after LayerZero OFT deploy. Set{" "}
          <code className="text-zinc-300">VITE_BCC_OFT_ADAPTER_ADDRESS</code> and{" "}
          <code className="text-zinc-300">VITE_BCC_BSC_OFT_ADDRESS</code> in env.
        </p>
        <p className="text-xs text-zinc-500">
          Same {BCC_SYMBOL} on both chains — 1:1 lock/mint, no new supply.
        </p>
      </div>
    );
  }
  ...
  async function handleBridge() {
    ...
    try {
      ...
      const fee = await writeContractAsync({
        address: sourceToken,
        abi: oftSendAbi,
        functionName: "send",
        args: [
  ...
  if (!isConnected || !address) {
    return (
      <div className={panelClass}>
        <p className="text-xs text-zinc-500">Connect a wallet to move {BCC_SYMBOL} between Base and BNB Chain.</p>
  ...
  return (
    <div className={panelClass}>
      ...
      <p className="text-xs text-zinc-400">
        Move the same {BCC_SYMBOL} 1:1 between chains. Supply stays unified — locks on source, mints
        on destination.
      </p>
      ...
      <div className="rounded-xl border border-white/10 bg-black/40 p-3">
        ...
        <p className="mt-1 text-[11px] text-zinc-500">
          Balance: {formatUnits(balance ?? 0n, decimals ?? 18)} {BCC_SYMBOL}
        </p>
      </div>
      ...
      >
        {pending ? "Bridging…" : `Bridge ${BCC_SYMBOL}`}
      </Button>
    </div>
  );
}
