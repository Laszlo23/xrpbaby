/**
 * Server-safe CEO orchestration — consumed by the BUILDCHAIN frontend and Temporal workers.
 */
import { AGENT_FLEET, CEO_AGENT_ID, TASK_ROUTE_HINTS, type FleetAgent } from "./fleet.js";
...
export function planOrchestration(taskKey: string): OrchestrationDecision {
  ...
  const ceo = AGENT_FLEET.find((a) => a.id === CEO_AGENT_ID) ?? AGENT_FLEET[0];
  if (!hint) {
    return {
      taskKey,
