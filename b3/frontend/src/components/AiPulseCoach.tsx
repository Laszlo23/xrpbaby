...
export function AiPulseCoach() {
  ...
  const { coachOpen, closeCoach } = useAiCoach();
        ...
        <DialogContent className="glass flex max-h-[85vh] flex-col border-white/[0.08] sm:max-w-lg sm:rounded-3xl">
          <DialogHeader className="shrink-0">
            ...
            <DialogDescription className="text-left text-zinc-500">
              AI tips for drops, XP, and BCD — not financial advice. On-chain facts may change;
          ...
          <div className="min-h-0 flex-1 space-y-3 overflow-y-auto py-2">
            {turns.length === 0 ? (
              <p className="text-sm leading-relaxed text-zinc-500">
                Ask anything about playing BUILDCHAIN, or tap a shortcut below. Your local XP /
                quest progress is sent as context when your wallet is connected (no full addresses
                stored server-side beyond a short hint).
              </p>
            ) : (
          ...
          <form
            onSubmit={onSubmit}
            className="flex shrink-0 gap-2 border-t border-white/[0.06] pt-3"
          >
            <Input
              value={input}
