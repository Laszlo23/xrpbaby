...
export function isLLMRateLimitError(err: unknown): boolean {
  ...
  if (typeof o.message === "string" && /\b429\b|529|rate.?limit|overloaded/i.test(o.message)) return true;
  const inner = o.error;
...
export function isLLMAuthError(err: unknown): boolean {
  ...
  if (typeof o.message === "string" && /authentication_error|invalid x-api-key|unauthorized/i.test(o.message))
    return true;
  const inner = o.error;
  if (inner && typeof inner === "object" && "type" in inner) {
    const t = (inner as { type?: string }).type;
